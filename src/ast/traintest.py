...
def train(audio_model, train_loader, test_loader, args, tracker=''):
    ...
    while epoch < args.n_epochs + 1:
        ...
        for i, (audio_input, labels) in enumerate(train_loader):
            ...
            with autocast():
                ...
                audio_output = m(audio_output)
                loss = loss_fn(audio_output, labels.long())
            ...
            _, predicted = torch.max(audio_output.data, 1)
            batch_y = torch.max(labels.data, 1)
            correct_train += (batch_y == predicted).sum().item()
            _acc = (100 * correct_train) / B
            ...
            if print_step and global_step != 0:
                print('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  'Train Accuracy {acc}'.format(
                   epoch, i, len(train_loader), per_sample_time=per_sample_time, per_sample_data_time=per_sample_data_time,
                      per_sample_dnn_time=per_sample_dnn_time, loss_meter=loss_meter, acc=_acc), flush=True)
        ...
        if tracker != '':
            tracker['eval/accuracy'].log(acc)
            # tracker['eval/mAUC'].log(mAUC)
            # tracker['eval/average_precision'].log(average_precision)
            # tracker['eval/average_recall'].log(average_recall)
            # tracker['eval/d_prime'].log(d_prime(mAUC))
            tracker['train/loss'].log(loss_meter.avg)
            tracker['eval/loss'].log(valid_loss)
...
def validate(audio_model, val_loader, args, epoch):
    ...
    with torch.no_grad():
        for i, (audio_input, labels) in enumerate(val_loader):
            ...
            loss = args.loss_fn(audio_output, labels.long())
