...
class AudiosetDataset(Dataset):
    ...
    def _wav2fbank(self, filename):
        ...
        fade = torchaudio.transforms.Fade(fade_in_len=10, fade_out_len=10) # pt
        vol = torchaudio.transforms.Vol(gain=0.1) #pt 
        # source : https://pytorch.org/audio/stable/transforms.html#pitchshift
        pitchSift = torchaudio.transforms.PitchShift(sr, 3) # pt
        ...
        if random.choice([True, False]):
            waveform = pitchSift(waveform)
        
        # source : https://pytorch.org/audio/stable/compliance.kaldi.html
        # {NOTE} : Remember to do save changes into the _wav2bank in AudioTestDataset
        fbank = torchaudio.compliance.kaldi.fbank(waveform, htk_compat=True, sample_frequency=sr, use_energy=False,
                                                  window_type='hanning', num_mel_bins=self.melbins, dither=0.0, frame_shift=10) # pt
    ...
    def __getitem__(self, index):
        ...
        fbank, sr = self._wav2fbank(datum['wav'])
        # for label_str in datum['labels'].split(','):
        #     label_indices[int(self.index_dict[label_str])] = 1.0
        label_indices[datum['labels']] += 1.0

        label_indices = torch.FloatTensor([datum['labels']]) # torch.FloatTensor(label_indices)

        # TODO : @nipdep ## Additional audio augmentations source : https://pytorch.org/audio/stable/functional.html#phase-vocoder
